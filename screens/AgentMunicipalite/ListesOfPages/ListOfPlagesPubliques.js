...
export default function ListOfPlagesPubliques({ navigation }) {
    const lienimage = '../../../images/'
    const [isRefreshing, setIsRefreshing] = useState(false);
    const viewRef = useRef(null);
    const plageService = new PlagePubliqueService()
    ...
    const GetAllPlages = () => {
        return new Promise((resolve, reject) => {
            plageService.GetAllPlagesPubliques()
                .then((res) => {
                    setPlages(res.data);
    ...
    const ContactItem = ({ item: { id_plage_publique }, index, animation }) => {
        ...
        return (
            ...
            >
                ...
                <TouchableOpacity style={styles.item}
                    onPress={() => navigation.navigate('DetailsPlagePublique', { id_plage_publique })}
                >

                    <View style={styles.avatar}>

                        <Image
                            source={require(lienimage + 'plage3.jpg')}
                            style={styles.image}
                            resizeMode="cover"
                        />
                    </View>
                    <View style={styles.details}>
                        <Text style={styles.name}>{id_plage_publique}</Text>
